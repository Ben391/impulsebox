...
if(isset($_GET["user_id"]) AND !empty($_GET["user_id"])) {
	...
	if($user_data = getUserData($user_id)) {

		$entry_id = getNestedValue($user_data, ["entry_data", "entry_id"]);

		$insured_person_first_name = getNestedValue($user_data, ["insured_person_data", "insured_person_first_name"]);
		$insured_person_last_name = getNestedValue($user_data, ["insured_person_data", "insured_person_last_name"]);
		$insured_person_street = getNestedValue($user_data, ["insured_person_data", "insured_person_street"]);
		$insured_person_phone = getNestedValue($user_data, ["insured_person_data", "insured_person_phone"]);
		$insured_person_salutation = getNestedValue($user_data, ["insured_person_data", "insured_person_salutation"]);
		$insured_person_zipcode = getNestedValue($user_data, ["insured_person_data", "insured_person_zipcode"]);
		$insured_person_city = getNestedValue($user_data, ["insured_person_data", "insured_person_city"]);
		$insured_person_birth_date = getNestedValue($user_data, ["insured_person_data", "insured_person_birth_date"]);
		$insured_person_email = getNestedValue($user_data, ["insured_person_data", "insured_person_email"]);
		$care_level = getNestedValue($user_data, ["insured_person_data", 'care_level']);
		$insurance_company_name = getNestedValue($user_data, ["insured_person_data", 'insurance_company_name']);
		$custom_insurance_company_name = getNestedValue($user_data, ["insured_person_data", 'custom_insurance_company_name']);
		$insurance_number = getNestedValue($user_data, ["insured_person_data", 'insurance_number']);
		$insurance_aid = getNestedValue($user_data, ["insured_person_data", 'insurance_aid']);

		$care_giver_person_first_name = getNestedValue($user_data, ["care_giver_person_data", "care_giver_person_first_name"]);
		$care_giver_person_last_name = getNestedValue($user_data, ["care_giver_person_data", "care_giver_person_last_name"]);
		$care_giver_person_street = getNestedValue($user_data, ["care_giver_person_data", "care_giver_person_street"]);
		$care_giver_person_zipcode = getNestedValue($user_data, ["care_giver_person_data", "care_giver_person_zipcode"]);
		$care_giver_person_city = getNestedValue($user_data, ["care_giver_person_data", "care_giver_person_city"]);
		$care_giver_person_phone = getNestedValue($user_data, ["care_giver_person_data", "care_giver_person_phone"]);
		$care_giver_person_mail = getNestedValue($user_data, ["care_giver_person_data", "care_giver_person_email"]);

		$care_service_name  = getNestedValue($user_data, ["care_giver_service_data", "care_giver_service_company"]);
		$care_service_street = getNestedValue($user_data, ["care_giver_service_data", "care_giver_service_street"]);
		$care_service_zipcode = getNestedValue($user_data, ["care_giver_service_data", "care_giver_service_zipcode"]);
		$care_service_city = getNestedValue($user_data, ["care_giver_service_data", "care_giver_service_city"]);

		$delivery_destination = getNestedValue($user_data, ["delivery_data", "delivery"]);

		$supplier_change = getNestedValue($user_data, ["delivery_data", "supplier_change"]);

		$product_full_data = getNestedValue($user_data, ["product_data", "product_full_data"]);
		$compilation_name = getNestedValue($user_data, ["product_data", "compilation_name"]);

		$bed_protectors_amount = getNestedValue($user_data, ["entry_data", "bed_protectors_amount"]);

	}

}

if ($custom_insurance_company_name = null) {
