					...
					if(count($dataRow) == count($expectedHeaders)) {
						...
						foreach($expectedHeaders as $key => $header) {
							...
							if(!empty($dataRow[$index])) {
								...
								if($key == 'insured_person_address_addition') {
									if(!isset($dataRow[$index]) OR empty($dataRow[$index])) {
										$dataRow[$index] = NULL;
									}
								}
								...
								if($key == 'supplier_change_delivery_start') {
									if(!isset($dataRow[$index]) OR empty($dataRow[$index])) {
										$dataRow[$index] = NULL;
									}
								}
						...
						$data["bed_protectors_amount"] = 0;
						/*$data["products"] = '{"1":{"quantity":"1","size":"","intolerance":""},"2":{"quantity":"1","size":"","intolerance":""},"3":{"quantity":"1","size":"","intolerance":""},"4":{"quantity":"1","size":"","intolerance":""},"5":{"quantity":"1","size":"","intolerance":""},"6":{"quantity":"1","size":"","intolerance":""},"7":{"quantity":"1","size":"","intolerance":""},"8":{"quantity":"1","size":"","intolerance":""},"9":{"quantity":"1","size":"","intolerance":""},"10":{"quantity":"1","size":"","intolerance":""},"11":{
						"quantity":"1","size":"","intolerance":""},"12":{"quantity":"1","size":"","intolerance":""},"13":{"quantity":"1","size":"s","intolerance":""}}';*/
						$data["products"] = '{}';
						...
						try {
							...
							if($entry_id = executeImport($mysqli, $data)) {
								...
								#$url .= '?file_name=test&entry_id=' . urlencode($entry_id) . '&signed_status=' . $signed_status . '&pdf_version='.$pdf_version . "&download_file=" . $download_file;
								#$pdfResult = file_get_contents($url);
								createPDF($entry_id, $signed_status, $pdf_version, $pdf_number, $download_file); 
							} else {
